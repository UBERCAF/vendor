...
function setup_vendor() {
    ...
    export UBER_ROOT="$3"
    if [ ! -d "$UBER_ROOT" ]; then
        echo "\$UBER_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$UBER_ROOT/$OUTDIR" ]; then
        mkdir -p "$UBER_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$UBER_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$UBER_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$UBER_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$UBER_ROOT"/vendor/uber/build/tools/smali/baksmali.jar
        export SMALIJAR="$UBER_ROOT"/vendor/uber/build/tools/smali/smali.jar
    fi
    ...
    if [ ! -f "$UBER_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$UBER_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$UBER_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$UBER_ROOT"/vendor/uber/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$UBER_ROOT"/"$OUTDIR"/radio
